...
import com.df.kia.service.DF3000Service;
import com.df.kia.service.DF5000Service;
import com.df.kia.service.Device3000ListDialog;
import com.df.library.entries.Measurement;
import com.df.kia.service.Device5000ListDialog;
...
public class CollectDataLayout extends LinearLayout {
    ...
    Runnable df3000Runnable = new Runnable() {
        ...
        public void run() {
            ...
            // 当接收到数据时的回调方法
            DF3000Service.OnReceiveData onReceiveData = new DF3000Service.OnReceiveData() {
                @Override
    ...
    private void collectData() {
        if (sDriver == null) {
            ...
            mDF3000Service = DF3000Service.instance(sDriver);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case Common.MESSAGE_STATE_CHANGE:
                    ...
                    switch (msg.arg1) {
                        // 已连接
                        case DF5000Service.STATE_CONNECTED:
                            showView(rootView, R.id.start_button, true);
                            showImage(true);
                            isConnected = true;
                            break;
                        // 正在连接
                        case DF5000Service.STATE_CONNECTING:
                            break;
                        // 监听中
                        case DF5000Service.STATE_LISTEN:
                            break;
                        // 待机
                        case DF5000Service.STATE_NONE:
                            break;
                        // 失去连接
                        case DF5000Service.STATE_CONNECTION_LOST:
    ...
    private void collectData(String message) {
        // 是否已经连接到设备
        if (mDF5000Service.getState() != DF5000Service.STATE_CONNECTED) {
